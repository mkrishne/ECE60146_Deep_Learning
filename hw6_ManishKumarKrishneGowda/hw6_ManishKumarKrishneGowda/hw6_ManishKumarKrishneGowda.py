...
Original file is located at
    https://colab.research.google.com/drive/1YZAOl9GG6vVj3v1PGZSqpIi-hQUPzNoA
"""
...
!wget -O RegionProposalGenerator-2.1.0.tar.gz \
    https://engineering.purdue.edu/kak/distRPG/RegionProposalGenerator-2.1.0.tar.gz?download
...
!tar -xvf /content/datasets_for_RPG.tar.gz -C /content/drive/MyDrive/hw6_data/RegionProposalGenerator-2.1.0/ExamplesObjectDetection

!tar -xvf /content/drive/MyDrive/hw6_data/RegionProposalGenerator-2.1.0/ExamplesObjectDetection/data/Purdue_Dr_Eval_Multi_Dataset-clutter-10-noise-20-size-10000-train.gz -C /content/drive/MyDrive/hw6_data/RegionProposalGenerator-2.1.0/ExamplesObjectDetection/data/
!tar -xvf /content/drive/MyDrive/hw6_data/RegionProposalGenerator-2.1.0/ExamplesObjectDetection/data/Purdue_Dr_Eval_Multi_Dataset-clutter-10-noise-20-size-1000-test.gz -C /content/drive/MyDrive/hw6_data/RegionProposalGenerator-2.1.0/ExamplesObjectDetection/data/
...
# %cd /content/drive/MyDrive/hw6_data/RegionProposalGenerator-2.1.0/ExamplesObjectDetection/
...
!unzip /content/drive/MyDrive/hw6_data/coco/annotations_trainval2017.zip -d /content/drive/MyDrive/hw6_data/coco/
    ...
    for i, cls in enumerate(classes):
        for j, ind in enumerate(indices):
            ...
            for box in annotations[cls][ind]["boxes"]:
                ...
                cv2.putText(
                    im, cls, (int(x), int(y - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2
                )
            axes[i][j].imshow(cv2.cvtColor(im, cv2.COLOR_BGR2RGB))  # Convert BGR to RGB for display
...
plot_sample_images(train_img_paths, train_img_anns, "Sample training images from COCO dataset")

# Plot sample validation images
plot_sample_images(val_img_paths, val_img_anns, "Sample validation images from COCO dataset")
...
# class CocoMultiObjectDetectionDataset is a custom dataset class designed for handling object detection task
class CocoMultiObjectDetectionDataset(torch.utils.data.Dataset):
    def __init__(self, root, paths, anns, max_objects = 5, transforms=None, mode = 'train'):
        ...
        #creatng grids and assigning their indices
        ...
        for c_h in self.yolocell_centers_h:
            for c_w in self.yolocell_centers_w:
                for w, h in self.anchor_box_shapes:
                    x = c_w - w / 2.0 #Calculates the x-coordinate of the top-left corner of the anchor box.
                    y = c_h - h / 2.0 #Calculates the y-coordinate of the top-left corner of the anchor box.
                    self.anchor_boxes.append([x, y, x+w, y+h]) #self.anchor_box_shapes is a list of pairs [width, height] representing different aspect ratios for the anchor boxes.
    ...
    def __getitem__(self, index):
        #Obtains the image path, opens the image, converts it to RGB, and applies any specified transformations.
        ...
        #Calculates the Intersection over Union (IOU) between the ground truth bounding boxes and anchor boxes.
        #Determines the index of the anchor box with the maximum IOU for each object.
        ...
        for i in range(num_objects_in_image):
            if bbox_label_tensor[i].item() == 4:
                continue
            yolo_cell_index[i] = max_ind[i] // 5 #Determine the YOLO cell index for each object based on the maximum intersection index.
            ind = max_ind[i]

            #Calculate Relative Position and Size (Delta) between Object and Anchor Box:
            ...
            #Using the logarithm allows the model to handle both small and large objects effectively,
...
transforms = tvt.Compose([
    tvt.ToTensor(),  # Convert to PyTorch tensor
    tvt.Resize((reshape_size, reshape_size)),  # Resize to desired size
    tvt.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))  # Normalize pixel values
])

# Specify the COCO dataset paths (modify according to your dataset location)
train_data_path = "/content/drive/MyDrive/hw6_data/coco/train2017"  # Replace with your train data path
val_data_path = "/content/drive/MyDrive/hw6_data/coco/val2017"  # Replace with your val data path
...
'''
The first dimension (64) corresponds to the batch size, meaning you have predictions for 64 different images in a single batch.
The second dimension (5) corresponds to the number of anchor boxes per image, indicating that the model predicts bounding boxes for 5 different anchor boxes.
The third dimension (8) corresponds to the elements of the prediction vector for each anchor box :
*   4 : bbox dimensions
*   1 : objectness score
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(data_loader):
            ...
            if (i+1) % display_interval == 0:
                ...
                print(f"[epoch: {epoch + 1}, batch: {i + 1}] loss: {avg_loss:.3f}, loss_1: {avg_loss_1:.3f}, loss_2: {avg_loss_2:.3f}, loss_3: {avg_loss_3:.3f}")
...
def plot_loss(loss_record, presence_loss_record, regression_loss_record,
    ...
    plt.plot(np.arange(len(presence_loss_record)) * display_interval, presence_loss_record,
             label="Object Presence Loss", color='green', linewidth=1.5)
    plt.plot(np.arange(len(regression_loss_record)) * display_interval, regression_loss_record,
             label="Bounding Box Regression Loss", color='orange', linewidth=1.5)
    plt.plot(np.arange(len(class_loss_record)) * display_interval, class_loss_record,
...
def plot_validation_results(imgs, gt_bboxes, gt_labels, pred_bboxes, pred_labels, plot_title="Ground Truth vs. Predicted Boxes"):
    """
    ...
    Args:
        ...
        plot_title (str, optional): Title for the visualization plot. Defaults to "Ground Truth vs. Predicted Boxes".
    """

    num_images_to_plot = 9  # Adjust as needed
    fig, axes = plt.subplots(3, 3, figsize=(12, 12))  # Increase figure size for better visualization

    for i in range(num_images_to_plot):
        image_idx = i % len(imgs)
        image = imgs[image_idx]  # Assuming the first element in the list is the image data
        image = (image * 0.5 + 0.5) * 255  # Normalize and convert to uint8
        image = np.ascontiguousarray(image.transpose(1, 2, 0), dtype=np.uint8)  # Convert to OpenCV format
        ...
        for box_idx in range(len(gt_bboxes[image_idx])):
            ...
            if len(pred_bboxes[image_idx])>0 and box_idx < len(pred_bboxes[image_idx]) and pred_bboxes[image_idx][box_idx][0] > 0 and pred_bboxes[image_idx][box_idx][1] > 0:
                X1, Y1, X2, Y2 = pred_bboxes[image_idx][box_idx]
                image = cv2.rectangle(image, (int(X1), int(Y1)), (int(X2), int(Y2)), (255, 0, 0), 2)
                image = cv2.putText(image, pred_labels[image_idx][box_idx], (int(X1), int(Y1 - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 1)
...
def load_previous_losses(model_name):
    loss_file = f'/content/drive/MyDrive/hw6_data/saved_models/{model_name}_losses.pkl'
    if os.path.exists(loss_file):
        import pickle
        with open(loss_file, 'rb') as f:
            prev_loss_running_record, prev_loss_1_running_record, \
                prev_loss_2_running_record, prev_loss_3_running_record = pickle.load(f)
...
if os.path.exists('/content/drive/MyDrive/hw6_data/saved_models/HW6Net_MSE2.pt'):
    net_mse.load_state_dict(torch.load('/content/drive/MyDrive/hw6_data/saved_models/HW6Net_MSE2.pt', map_location=device))
...
net_mse_losses = train_net(device, net_mse, optimizer=optimizer, data_loader=train_data_loader,criterion_bbox=criterion_bbox,
                        model_name='HW6Net_MSE2', epochs=epochs, display_interval=display_interval)

net_mse_losses = list(net_mse_losses)
# Load previous loss records for 'HW6Net_MSE2' (replace with your actual model name)
prev_loss_running_record, prev_loss_1_running_record, prev_loss_2_running_record, prev_loss_3_running_record = load_previous_losses('HW6Net_MSE2')
...
plot_loss(net_mse_losses[0][:400], net_mse_losses[1][:400], net_mse_losses[2][:400], net_mse_losses[3][:400], display_interval, 'HW6Net_CIOU')
#plot_loss(net_mse_losses[0][50:], net_mse_losses[1][50:], net_mse_losses[2][50:], net_mse_losses[3][50:], display_interval, 'HW6Net_MSE2')

plot_loss(net_mse_losses[0][:100], net_mse_losses[1][:100], net_mse_losses[2][:100], net_mse_losses[3][:100], display_interval, 'HW6Net_CIOU')
...
# Load saved model weights if desired (uncomment and adjust path)
net_mse.load_state_dict(torch.load('/content/drive/MyDrive/hw6_data/saved_models/HW6Net_MSE2.pt', map_location=device))
imgs, gt_bboxes, gt_labels, pred_bboxes, pred_labels = validate_net(device, net_mse, val_data_loader, model_path = save_path)
img_list = [imgs[0][i] for i in range(imgs[0].shape[0]) ]
print(img_list[0].shape)
plot_validation_results(img_list, gt_bboxes[0], gt_labels, pred_bboxes, pred_labels, plot_title="Gnd Truth vs. Pred Boxes with HW6Net_MSE")
...
if os.path.exists('/content/drive/MyDrive/hw6_data/saved_models/HW6Net_CIOU.pt'):
    net_ciou.load_state_dict(torch.load('/content/drive/MyDrive/hw6_data/saved_models/HW6Net_CIOU.pt', map_location=device))
...
net_ciou_losses = train_net(device, net_ciou, optimizer=optimizer, data_loader=train_data_loader,criterion_bbox=criterion_bbox,
                        model_name='HW6Net_CIOU', epochs=epochs, display_interval=display_interval)

net_ciou_losses = list(net_ciou_losses)
# Load previous loss records for 'HW6Net_MSE2' (replace with your actual model name)
prev_loss_running_record, prev_loss_1_running_record, prev_loss_2_running_record, prev_loss_3_running_record = load_previous_losses('HW6Net_CIOU')
...
# Plotting HW6Net_MSE training loss
plot_loss(net_ciou_losses[0][50:], net_ciou_losses[1][50:], net_ciou_losses[2][50:], net_ciou_losses[3][50:], display_interval, 'HW6Net_CIOU')
...
# Load saved model weights if desired (uncomment and adjust path)
net_ciou.load_state_dict(torch.load('/content/drive/MyDrive/hw6_data/saved_models/HW6Net_CIOU.pt', map_location=device))
imgs, gt_bboxes, gt_labels, pred_bboxes, pred_labels = validate_net(device, net_ciou, val_data_loader, model_path = save_path)
img_list = [imgs[0][i] for i in range(imgs[0].shape[0]) ]
print(img_list[0].shape)
plot_validation_results(img_list, gt_bboxes[0], gt_labels, pred_bboxes, pred_labels, plot_title="Gnd Truth vs. Pred Boxes with HW6Net_MSE")